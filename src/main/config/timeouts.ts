 ...
 */
export const DEFAULT_TIMEOUT_BUDGETS: Record<TaskMode, number> = {
  general: 60_000,
  writing: 60_000,
  coding: 300_000,
  image: 120_000,
...
export interface TimeoutSettings {
  modeBudgets?: Partial<Record<TaskMode, number>>;
  providerOverrides?: Partial<Record<ProviderId, Partial<Record<TaskMode, number>>>>;
}
...
export class TimeoutManager {
  ...
  public static getTimeout(mode: TaskMode, providerId?: ProviderId): number {
    if (providerId && this.customSettings.providerOverrides?.[providerId]?.[mode]) {
      return this.customSettings.providerOverrides[providerId]![mode]!;
    }
    if (this.customSettings.modeBudgets?.[mode]) {
      return this.customSettings.modeBudgets[mode]!;
    }
    return DEFAULT_TIMEOUT_BUDGETS[mode] || 60_000;
  }
}
