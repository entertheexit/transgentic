...
3. Enable the service and select it in the existing General or Coding route. Writing requests retain Writing mode and guidance while using the General route configuration. Quick Prompt, MCP, and OpenAI-compatible route models follow those routes without a separate Hub selector. Webview, API, CLI, and local LLM services share the routing model. CLI services do not support image, video, or music generation routes.
4. Optionally choose a native model in the service settings. Transgentic asks Codex, Antigravity, and Grok for their native model catalogs and shows the existing styled selector when discovery succeeds. Claude Code, unsupported versions, empty catalogs, and failed discovery retain the manual field. **CLI default** clears the override, and **Custom model…** accepts a model that is absent from the discovered catalog. Provider web-model catalogs are not reused as verified CLI models.
